...
contains
    ...
    function   decompose_j(j, Nvec) result(n)
        ...
        if (j < 1 .or. j > calc_numb_coefs(Nvec)) then
            write(*,*) 'Error decomposing j '//int2str(j)//' into n for Nvec with '&
            //int2str(calc_numb_coefs(Nvec))//' allowable coefficients! Quitting...'
            stop
        end if
    ...
    function   compose_j(n, Nvec) result(j)
        ...
        if (size(n) /= VV) then
            write(*,*) 'Error composing j for n and Nvec. Size of Nvec '&
            //int2str(VV)//' does not equal size of n '//int2str(size(n))//'! Quitting...'
            stop
        end if
    ...
    subroutine database_init(t, fn, pn)
        ...
        outer: do
            ...
            if (trim(cols(1)) == 'parameter') then
                ...
                headings_row = n_rows
                if (t%n_iv == 0) then
                    write(*,*) 'Error reading database csv '&
                    //trim(input_file)//'! Number of independent variables not given! Quitting...'
                    close(fid)
                    stop
                end if
        ...
        if (n_rows /= n_comments + n_parameters + 1 + n_pts) then
            write(*,*) 'Error reading database csv '//trim(input_file)&
            //'! Number of rows, comments, parameters, and data points do not agree!'
    ...
    function   new_database(db_type, fn, pn) result(obj)
        ...
        select case (db_type)
            ...
            case('recursive', 'Recursive')
                ...
                ! obj = temp_recu
                write(*,*) 'Error initializing database from file &
                '//trim(fn)//'. Recursive database type currently not available! Quitting...'
                stop
            case('scatter', 'Scatter')
                ...
                ! obj = temp_scat
                write(*,*) 'Error initializing database from file &
                '//trim(fn)//'. Scatter database type currently not available! Quitting...'
                stop
            case default
                write(*,*) 'Error initializing database from file &
                '//trim(fn)//'. Database type '//trim(db_type)//' not recognized!'
                write(*,*) 'Must be either rectilinear, recursive, &
                or scatter (currently only rectilinear is available). Quitting...'
                stop
        end select
